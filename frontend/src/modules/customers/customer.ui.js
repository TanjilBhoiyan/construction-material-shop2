...
// ===========================================================
// ⚠️ পুরনো dead code — booking.ui.js আলাদা রাখার সিদ্ধান্তের পর এটা আর ব্যবহার হয় না,
// কোথাও import/কল হয় না। এখনকার কাজের সাথে সংঘর্ষ নেই, কিন্তু মুছে ফেলা যায় চাইলে।
// ===========================================================
...
async function populateBookingProductDropdown() {
    ...
    try {
        ...
        if (bookingProducts.length > 0) {
            const lockedPriceInput = document.getElementById('booking-locked-price');
            if (lockedPriceInput) lockedPriceInput.value = bookingProducts[0].default_selling_price;
        }
    } catch (err) {
...
function updateBookingTotal() {
    const qty = parseFloat(document.getElementById('booking-qty')?.value) || 0;
    const price = parseFloat(document.getElementById('booking-locked-price')?.value) || 0;
...
async function handleBookingSubmit(e) {
    ...
    const bookingData = {
        customerName: document.getElementById('booking-cust-name')?.value.trim() || '',
        ...
        quantity: parseFloat(document.getElementById('booking-qty')?.value) || 0,
        lockedPrice: parseFloat(document.getElementById('booking-locked-price')?.value) || 0,
        advancePaid: parseFloat(document.getElementById('booking-advance-paid')?.value) || 0
    };

    if (!bookingData.customerName || !bookingData.productId || bookingData.quantity <= 0 || bookingData.lockedPrice <= 0) {
        ToastUI.showToast("দয়া করে সব প্রয়োজনীয় ঘর সঠিকভাবে পূরণ করুন।", true);
        return;
    }
    ...
    try {
        ...
        ToastUI.showToast(`🎉 "${bookingData.customerName}"-এর বুকিং সফলভাবে সংরক্ষিত হয়েছে!`);
        resetBookingForm();
    } catch (err) {
...
export function initBookingModule() {
    ...
    if (productSelect) {
        productSelect.addEventListener('change', () => {
            const selected = bookingProducts.find(p => p.id == productSelect.value);
            const lockedPriceInput = document.getElementById('booking-locked-price');
            if (selected && lockedPriceInput) lockedPriceInput.value = selected.default_selling_price;
            updateBookingTotal();
        });
    }
    ...
    if (priceInput) priceInput.addEventListener('input', updateBookingTotal);
